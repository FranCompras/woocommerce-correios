...
Utilize os métodos de entrega e serviços dos Jadlog com a sua loja WooCommerce.

[Jadlog](http://www.jadlog.com.br/) é um método de entrega brasileiro.

O plugin WooCommerce Jadlog foi desenvolvido sem nenhum incentivo dos Jadlog. Nenhum dos desenvolvedores deste plugin possuem vínculos com esta empresa. E note que este plugin foi feito baseado na documentação do [Webservices Jadlog](http://www.jadlog.com.br/webservices/) e com apoio da [Infranology](http://infranology.com.br/) na construção das classes de cubagem.
...
- Entrega nacional:
 ...
 - SEDEX 12
 - SEDEX Hoje
 - e-SEDEX (necessário conta administrativa nos Jadlog)
 - Carta Registrada
 - Impresso Normal
 - Impresso Urgente
...
Confira o nosso guia de instalação e configuração do Jadlog na aba [Installation](http://wordpress.org/extend/plugins/woocommerce-jadlog/installation/).
...
- A nossa sessão de [FAQ](http://wordpress.org/extend/plugins/woocommerce-jadlog/faq/).
- Utilizando o nosso [fórum no Github](https://github.com/claudiosanches/woocommerce-jadlog).
- Criando um tópico no [fórum de ajuda do WordPress](http://wordpress.org/support/plugin/woocommerce-jadlog).
...
Alternativamente, você pode configurar apenas o peso e deixar as dimensões em branco, pois serão utilizadas as configurações do **Pacote Padrão** para as dimensões (neste caso pode ocorrer uma variação pequena no valor do frete, pois os Jadlog consideram mais o peso do que as dimensões para a cotação).
...
É possível configurar os dois em "WooCommerce" > "Configurações" > "Integrações" > "Jadlog".

= Como alterar a mensagem que é enviada no e-mail do código de rastreamento? =

É possível encontrar configurações para o e-mail do código de rastreamento em "WooCommerce" > "Configurações" > "E-mails" > "Código de rastreio dos Jadlog".

= Como é feita a cotação do frete? =

A cotação do frete é feita utilizando o [Calculador Remoto de Preços e Prazos dos Jadlog](https://www.jadlog.com.br/para-sua-empresa/servicos-para-o-seu-contrato/precos-e-prazos).
...
Escopo deste plugin é prover integração entre o WooCommerce e os Jadlog.

= Este plugin faz alterações na calculadora de frete na página do carrinho ou na de finalização? =

Não, nenhuma alteração é feita, este plugin funcionando esperando o WooCommerce verificar pelos valores de entrega, então é feita uma conexão com os Jadlog e os valores retornados são passados de volta para o WooCommerce apresentar.
...
Mesmo você configurando os métodos de entrega, eles não são exibidos quando os Jadlog retornam mensagens de erro, por exemplo quando a região onde o cliente esta não é coberta pelos Jadlog ou quando o peso do pacote passa do limite suportado.
...
Dica: Caso apareça no log a mensagem `WP_Error: connect() timed out!` pode acontecer do site dos Jadlog ter caído ou o seu servidor estar com pouca memoria.

= Os métodos de entrega dos Jadlog não aparecem no carrinho ou durante a finalização? =

As mesmas dicas da sessão acima valem como solução para isto também.

= O valor do frete calculado não bateu com a da loja dos Jadlog? =

Este plugin utiliza o Webservices dos Jadlog para calcular o frete e quando este tipo de problema acontece geralmente é porque:
...
3. O Webservices dos Jadlog enviou um valor errado! Sim isso acontece e na página da documentação do Webservice tem o seguinte alerta:
...
Se estiver tendo problemas, antes de tudo ative a opção de **Log de depuração** do método que você esta tendo problema e tente novamente cotar o frete, fazendo isso, um arquivo de log é criado e são registradas as respostas do Webservice dos Jadlog, leia o arquivo de log, nele é descrito exatamente o que esta acontecendo, tanto o que foi concluindo com sucesso ou não.

Se ainda não foi capaz de solucionar o problema, copie o conteúdo do arquivo de log, cole no [pastebin.com](http://pastebin.com), salve e pegue o link gerado, depois disso abra um tópico informando o seu problema no [fórum de suporte do plugin](https://wordpress.org/support/plugin/woocommerce-jadlog/#new-post).

= Dúvidas sobre o funcionamento do plugin? =

Em caso de dúvidas, basta abrir um tópico no [fórum de suporte do plugin](https://wordpress.org/support/plugin/woocommerce-jadlog/#new-post), vou responder conforme eu tenho tempo livre e caso sua dúvida for relacionada com o funcionamento deste plguin.

== Screenshots ==

1. Exemplo de áreas de entrega com os Jadlog.
2. Exemplo da tela de configurações dos métodos de entrega.
3. Configurações de integração com os Jadlog.
...
- Atualizada a forma que são exibidos os códigos de rastreamento, uma vez que os Jadlog desativou o link que era utilizado.
...
- Adicionado objeto do pacote no filtro `woocommerce_jadlog_{$method_id}_rate`.

= 3.1.4 - 2016/12/23 =

- Adicionada prevenção de erros no PHP quando todos os valores de peso e dimensões estão em branco.
- Introduzido o filtro `woocommerce_jadlog_get_estimating_delivery` para edição da estimativa de entrega.
- Adicionada informações do pacote nos filtros para tornar possível calcular frete para múltiplos pacotes.
- Adicionada prevenção de erro quando Jadlog retorna apenas um objeto no rastreio de entrega.
...
- Introduzida a função `wc_jadlog_get_address_by_postcode()`.
...
- Corrigido o tratamento do XML das métodos internacionais, nos quais os Jadlog as vezes envia um XML no formato incorreto.
...
- Adicionado novo método de Carta Registrada seguindo a tabela dos Jadlog e com suporte a classes de entrega.
...
- Integrado o campo de "jadlog_tracking_code" dos pedidos com a API REST do WooCommerce.
- E mais outras várias alterações que podem ser verificadas pelo [GitHub](https://github.com/claudiosanches/woocommerce-jadlog/compare/2.3.0...3.0.0).
