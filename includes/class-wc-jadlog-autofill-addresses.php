<?php
/**
 * Jadlog Autofill Addresses.
 *
 * @package WooCommerce_Jadlog/Classes/Autofill
 * @since   3.0.0
 * @version 3.0.0
 */
...
/**
 * Jadlog autofill addresses class.
 */
class WC_Jadlog_Autofill_Addresses {
	 ...
	 * @var string
	 */
	public static $table = 'jadlog_postcodes';
	 ...
	 * @var string
	 */
	protected $ajax_endpoint = 'jadlog_autofill_address';
	 ...
	 * @var string
	 */
	private static $_webservice_url = 'https://apps.jadlog.com.br/SigepMasterJPA/AtendeClienteService/AtendeCliente?wsdl';
	...
	public function init() {
		if ( apply_filters( 'woocommerce_jadlog_enable_autofill_addresses', false ) ) {
			$this->maybe_install();
	...
	protected static function get_tracking_addresses_webservice_url() {
		return apply_filters( 'woocommerce_jadlog_addresses_webservice_url', self::$_webservice_url );
	}
	...
	protected static function logger( $data ) {
		if ( apply_filters( 'woocommerce_jadlog_enable_autofill_addresses_debug', false ) ) {
			$logger = new WC_Logger();
			$logger->add( 'jadlog-autofill-addresses', $data );
		}
	}
	...
	protected static function get_validity() {
		return apply_filters( 'woocommerce_jadlog_autofill_addresses_validity', 'forever' );
	}
	...
	public static function get_address( $postcode ) {
		...
		$postcode = wc_jadlog_sanitize_postcode( $postcode );
	...
	/**
	 * Fetch an address from Jadlog Webservices.
	 *
	 * @param string $postcode Address postcode.
	 * @return stdClass
	 */
	protected static function fetch_address( $postcode ) {
		include_once dirname( __FILE__ ) . '/class-wc-jadlog-soap-client.php';

		self::logger( sprintf( 'Fetching address for "%s" on Jadlog Webservices...', $postcode ) );

		$address = null;

		try {
			$soap     = new WC_Jadlog_Soap_Client( self::get_tracking_addresses_webservice_url() );
	...
	public function frontend_scripts() {
		if ( is_checkout() || is_account_page() ) {
			...
			wp_enqueue_script( 'woocommerce-jadlog-autofill-addresses', plugins_url( 'assets/js/frontend/autofill-address' . $suffix . '.js', WC_Jadlog::get_main_file() ), array( 'jquery', 'jquery-blockui' ), WC_JADLOG_VERSION, true );

			wp_localize_script(
				'woocommerce-jadlog-autofill-addresses',
				'WCJadlogAutofillAddressParams',
				array(
					'url'   => WC_AJAX::get_endpoint( $this->ajax_endpoint ),
					'force' => apply_filters( 'woocommerce_jadlog_autofill_addresses_force_autofill', 'no' ),
				)
			);
		}
	}
	...
	public function ajax_autofill() {
		if ( empty( $_GET['postcode'] ) ) { // WPCS: input var okay, CSRF ok.
			wp_send_json_error( array( 'message' => __( 'Missing postcode paramater.', 'woocommerce-jadlog' ) ) );
			exit;
		}

		$postcode = wc_jadlog_sanitize_postcode( wp_unslash( $_GET['postcode'] ) ); // WPCS: input var okay, CSRF ok.

		if ( empty( $postcode ) || 8 !== strlen( $postcode ) ) {
			wp_send_json_error( array( 'message' => __( 'Invalid postcode.', 'woocommerce-jadlog' ) ) );
			exit;
		}
	...
	public function maybe_install() {
		$version = get_option( 'woocommerce_jadlog_autofill_addresses_db_version' );

		if ( empty( $version ) ) {
			self::create_database();

			update_option( 'woocommerce_jadlog_autofill_addresses_db_version', '1.0.0' );
		}
	}
...
new WC_Jadlog_Autofill_Addresses();
