...
class WC_Jadlog_REST_API {
	...
	public function legacy_orders_response( $data, $order, $fields ) {
		$data['jadlog_tracking_code'] = implode( ',', wc_jadlog_get_tracking_codes( $order ) );
	...
	public function legacy_orders_update( $order_id, $data ) {
		if ( isset( $data['jadlog_tracking_code'] ) ) {
			wc_jadlog_update_tracking_code( $order_id, $data['jadlog_tracking_code'] );
		}
	}
	...
	public function register_tracking_code() {
		...
		register_rest_field( 'shop_order',
			...
			array(
				...
				'schema'          => array(
					'description' => __( 'Jadlog tracking code.', 'woocommerce-jadlog' ),
					'type'        => 'string',
	...
	function get_tracking_code_callback( $data, $field, $request ) {
		return implode( ',', wc_jadlog_get_tracking_codes( $data['id'] ) );
	}
	...
	function update_tracking_code_callback( $value, $object ) {
		...
		return wc_jadlog_update_tracking_code( $object->ID, $value );
	}
}

new WC_Jadlog_REST_API();
