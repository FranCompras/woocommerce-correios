<?php
/**
 * Jadlog Tracking History.
 *
 * @package WooCommerce_Jadlog/Classes/Tracking
 * @since   3.0.0
 * @version 3.2.0
 */
...
/**
 * Jadlog tracking history class.
 */
class WC_Jadlog_Tracking_History {
	 ...
	 * @var string
	 */
	private $_webservice_url = 'https://webservice.jadlog.com.br/service/rastro/Rastro.wsdl';
	...
	protected function get_tracking_history_webservice_url() {
		return apply_filters( 'woocommerce_jadlog_tracking_webservice_url', $this->_webservice_url );
	}
	...
	protected function get_user_data() {
		$user_data = apply_filters( 'woocommerce_jadlog_tracking_user_data', array( 'login' => 'ECT', 'password' => 'SRO' ) );
	...
	protected function logger( $data ) {
		if ( apply_filters( 'woocommerce_jadlog_enable_tracking_debug', false ) ) {
			$logger = new WC_Logger();
			$logger->add( 'jadlog-tracking-history', $data );
		}
	}
	...
	protected function get_tracking_history( $tracking_codes ) {
		...
		$this->logger( sprintf( 'Fetching tracking history for "%s" on Jadlog Webservices...', implode( ', ', $tracking_codes ) ) );

		$objects   = null;
		$user_data = $this->get_user_data();
		$args      = apply_filters( 'woocommerce_jadlog_tracking_history_webservice_args', array(
		...
		try {
			$soap     = new WC_Jadlog_Soap_Client( $this->get_tracking_history_webservice_url() );
			$response = $soap->buscaEventos( $args );

			// Handle Jadlog multiple formats response.
		...
		return apply_filters( 'woocommerce_jadlog_tracking_objects', $objects, $tracking_codes );
	}
	...
	public function view( $order ) {
		...
		// Try to connect to Jadlog Webservices and get the tracking history.
		if ( apply_filters( 'woocommerce_jadlog_enable_tracking_history', false ) ) {
...
new WC_Jadlog_Tracking_History();
